...
その代わり、**ナレーションの表記が読み上げと字幕の両方を決めます**。指数は実測で
次のように振る舞いました（ja-JP-NanamiNeural、`[]` は返ってきた語の区切り）:
...
字幕は `t=x²+2x` のように組み上がります。`たす` は `にじょうたす` のように
前の語と癒着して返ってくるため、トークンの途中でも変換しますが、**左側が数字・英字・
記号のときだけ**です。この条件が「満たす」を「満+」にしないための境界線です。
