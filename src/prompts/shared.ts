...
- 分数は必ず **「分母＋分の＋分子」の語順**で書く。分数の意味でスラッシュや LaTeX 記法を使わない。
  1/2 →「2分の1」、π/4 →「4分のパイ」、x/2 →「2分のx」、(3√19)/4 →「4分の3ルート19」。
  「分の」で範囲を明示すると読みが安定し、字幕でも分数全体を1トークンにまとめて上下に組める。
- 根号 √7 →「ルート7」
...
- + - × ÷ → 「たす」「ひく」「かける」「わる」（正負の符号は「プラス」「マイナス」）。
  普通の動詞は漢字にする。良い例:「対角線を引く」「条件を満たす」。悪い例:「対角線をひく」。
- x^2 → 「xのにじょう」、x^3 → 「xのさんじょう」、x^4 →「xのよんじょう」。**ひらがなで書き、「の」を省かない**
  「2乗」「二乗」と書くと数字と「乗」に分断され、単独の「乗」が「の」と読まれる。
  「の」を省いた「xにじょう」も「に」と「じょう」に割れる。「ACのにじょう」も同じ
- 指数が式なら 2^(n+1) →「2のnたす1じょう」。字幕は「2のn+1乗」とし、複雑な指数は画面の数式行で示す。
- cos sin は綴りのまま1文字ずつ読まれてしまう → 「コサイン」「サイン」と書く
  （tan, log はそのままで正しく読まれる。tan を「タンジェント」と書いた場合も字幕では tan に戻る）
- 極限 lim_{n→∞} は narration では「エヌがむげんだいのときのリミット」と書く。∞、_、→を
  入れずに読みを語に開き、字幕では「リミット」が lim に戻る。
- ギリシャ文字は θ→「シータ」、π→「パイ」、α→「アルファ」、β→「ベータ」、γ→「ガンマ」、
  δ→「デルタ」、λ→「ラムダ」、ω→「オメガ」、総和の ∑ または Σ→「シグマ」と書く。字幕では記号に戻る。
- **数列の添字の _ は「アンダーライン」と声に出して読まれる。narration には書かない**
  a_n →「エーエヌ」、a_1 →「エーイチ」、S_n →「エスエヌ」、
  a_{n+1} →「エーエヌプラスイチ」、a_{n-1} →「エーエヌマイナスイチ」とカタカナで書く。
  対応する文字は a,b,c,d,S,T,P（エー、ビー、シー、ディー、エス、ティー、ピー）、
  添字は n,k,m,1,2,3,4（エヌ、ケー、エム、イチ、ニ、サン、ヨン）と n+1,n-1。
  下線を外した「an」は「案」と読まれるので、それも書かない。
  （字幕では自動で aₙ, a₁, Sₙ, aₙ₊₁ に戻して表示される）
- 数字とカタカナを直接つなげない。「98コサインB」ではなく「98かけるコサインB」。
  つなげると読み上げの語の切れ目がずれる

字幕では かっこ1→(1)、プラスマイナス→±、コサイン→cos、イコール→=、かける→×、シータ→θ、パイ→π
のように自動で書き言葉に戻して表示される。式の「xのにじょうたす2x」は「x²+2x」、
操作を述べる「両辺をにじょうすると」は「両辺を2乗すると」になる。
