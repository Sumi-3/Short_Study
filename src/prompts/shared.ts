...
const VISUAL_DOCS = {
  ...
  formula: `- "formula": 定義式・公式・式変形を見せる
    ...
    1行でもマーカーを使ったら、そのシーンの式全体で ↓ と最後の自動囲みは出ない。
  ...
  plot: `- "plot":    座標平面。グラフ・曲線・領域を見せる
    ...
    visual_items = グラフと同時に見せる短い LaTeX 行を0〜2個（不要なら空配列）。図の下に表示される。
  ...
  figure: `- "figure":  図形そのものを描く。三角形・円・立体など、幾何の問題では必ず使う
    ...
    visual_items = 図と同時に見せる短い LaTeX 行を0〜2個（不要なら空配列）。図の下に表示される。
...
未使用のフィールドは必ず空（空配列 / 空文字列）にする。${
    bothMathKinds
      ? `
LaTeX と expr は別物。formula と figure / plot の visual_items は LaTeX。
