...
- **LaTeX・MathJax の記法は残さない。** ただし分数の \\frac{…}{…}、添字と指数の
  _ と ^ は画面で組版するため残す。$ は残さず、バックスラッシュは \\frac の先頭だけ残す。
    $\\sqrt{2}$ → √2    $\\frac{\\pi}{4}$ → \\frac{π}{4}    $\\theta$ → θ
    $\\le$ → ≦          $\\sin 2\\theta$ → sin2θ    $[ \\ ア \\ ]$ → ［ア］
- **数学の分数は \\frac{分子}{分母} にする。** 3√19/4 → \\frac{3√19}{4} のように、
  分子・分母の範囲を意味に沿って明示する。単位の km/h、日付など、分数でないスラッシュは変えない。
...
- **極限と総和は画面用の記法にする。** $\\lim_{n \\to \\infty}$ → lim_{n→∞}、
  $\\sum_{k=1}^{n} k$ → ∑_{k=1}^{n} k。\\lim、\\sum、\\to、\\infty は残さない。
...
- 数式は topic と同じ表記のまま写す（\\frac{…}{…}、_ ^ 記号、lim_{n→∞}、∑_{k=1}^{n} を含めて）。
  極限・総和でも \\lim や \\sum に戻さず、式の途中で分断しない。
- 問題に無い条件・解き方・答えは足さない。
    ...
    outline: "(1) ∫_0^π \\frac{x sinx}{1+cos^2x} dx を求めよ。"
