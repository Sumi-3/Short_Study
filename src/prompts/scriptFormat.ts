...
- 英字: 1文字も並びも、x、a、A、P、AB、AC、ABC とそのまま書く →「エックス」「エー」「ピー」等。
  **カタカナ読みにしない**（字幕は読み上げ原稿がそのまま出るので、「ピー」と書けば「ピー」と表示される）。
  点P、線分AB、係数a のように文字が単独で出るときは必ず英字で書く。
  **例外は添字の付く数列の項だけ**で、下の規則どおりカナにする。
  a_n は「エーエヌ」だが、添字の無い P は「ピー」ではなく P と書く。
...
- **数列の添字の _ は「アンダーライン」と声に出して読まれる。narration には書かない**
  ...
  **このカナ書きは添字とセットのときだけ**である。同じ文字でも添字が無ければ英字で書く。
...
- 数式を含めるなら $...$ で囲む（$\\cos B$ の求め方）。地の文は囲まない。
  ...
  table のセル、flow の step、図とグラフの label。narration だけは例外で $ を書かない。
