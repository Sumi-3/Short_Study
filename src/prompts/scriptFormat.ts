...
- シーン数・合計文字数・再生時間の上限は設けない。必要な説明が済んだら終える。
...
- 英字: 1文字も並びも、x、a、A、AB、AC、ABC とそのまま書く →「エックス」「エー」「エービー」等。
  **カタカナ読みにしない**（字幕は読み上げ原稿がそのまま出るので、「エー」と書けば「エー」と表示される）。
  添字の付く数列の項だけは例外で、下の規則どおりカナで書く
...
- **番号付き方針を使わない動画で、前のシーンの続きなら visual_content は空文字 "" にする。**
...
const VISUAL_DOCS = {
  ...
  formula: `- "formula": 定義式・公式・式変形と、その理由を見せる
    ...
    **[text] 行の中の数式は $...$ で囲む。** 行全体が数式なら [text] を使わず数式行にする。
    ...
    かつ前の最終数式をそのまま整理・計算する場合だけ、現在の visual_content は方針の番号付きタイトルを保ち、
    番号付き方針を使わない動画では空文字にする。そのうえで、
...
- 問題に無い条件・解き方・答えは足さない。解く方針の①②③もここには書かない。
  outline は最初の問題文専用であり、各設問の方針は別のpoint / bulletsシーンに置く。
