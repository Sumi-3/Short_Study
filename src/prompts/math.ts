...
- 最終的な答えは画面と音声の両方で言い切る。答えを画面だけ・音声だけにしない。
  図が答えの根拠なら figure / plot に式を併記してよい。画面での書き方は下の formula を見る。
...
- 画面の文章はそのまま読み上げない。前後の式を使って理由を補い、式の読み下しは必要な箇所に絞る。
...
- [strike] で候補を除くときは、除外の理由を必ず音声で言う。正しい等式変形を打ち消さない。
- マーカーは visual_items の行頭だけに書く。narration や図の label、plot の expr には入れない。
 ...
 * 1ファイルに集めると重複が目で見えるようになる。入口を探しやすいよう mathPrompt() は
 * 先頭に残してある。
 *
 * 見えた重複は畳んである。1つの規則は1箇所にしか書かない。言い回しの違う写しが増えると、
 * モデルへ矛盾した目標を渡すことになり、直すときに他の箇所が残るためである。正本の場所:
 *
 *   $...$ で囲む範囲            VISUAL_CONTENT（画面に出る文字列すべてに共通）
 *   問題文の組版                TOPIC_RULE
 *   行数・装飾マーカー・[substitute:]・[carry]・答えの書き方
 *                               VISUAL_DOCS.formula
 *   方針シーンと解説タイトルの対応  mathPrompt() の「# 方針シーン」
 *   ナレーションと画面の役割分担    narrationRules()
 *
 * mathPrompt() 本体に置くのは「いつどの種別を使うか」と単元ごとの必須図、つまり
 * フィールドの書式ではなく編集上の判断だけにする。
 *
 * 書式の規則のうち機械で確かめられるものは sceneIntegrity.ts が却下する。ただし却下は
 * 強制であって周知ではないので、ここから規則そのものを消してはいけない。書いていない規則を
 * 破らせてから却下すると、生成を1回無駄にするだけになる。
 * ------------------------------------------------------------------ */
...
## 字幕用ブロック
- 画面で読む文章を先に書く。数式の $...$ と LaTeX は下の visual_content の規則どおりに書く。
...
const VISUAL_DOCS = {
  bullets: `- "bullets": 並列な要素を列挙する
    visual_items = 項目数は内容に応じて可変。項目を削って4個に収めない。
    丸数字は㊿まで、それを超える場合は「51. 」の形式を使う。
    方針の書き方は上の「方針シーン」に従う。通常の箇条書きは短い要点にする`,
  ...
  formula: `- "formula": 定義式・公式・式変形と、その理由を見せる
    ...
    [text] 行は日本語の文なので、$ の外は x>0 のような本文表記にし、LaTeX コマンドを裸で書かない（topic と同じ規則）。
