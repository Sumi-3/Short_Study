 ...
 * （Studio のプレビューと web アプリの player）は 900 を W8 で打ち止めにするので見た目が変わらず、
 * 一見問題が無いように読める。しかし `remotion render` が使う Chrome Headless Shell は、その
 * W8 へさらに synthetic bold を重ねる。48px の和文見出しで濃い画素が 24.9%→28.7% に増え、
 * 画どうしが繋がって「動」の中の白場が塞がった。**プレビューでは再現しない**ので、mp4 へ書き出す
 * 予定があるなら 800 を上限として扱う。
 */
